...
Write-Step "Series Trailer Downloader dependency bootstrap"
...
Invoke-Logged $pythonExe ($pythonArgs + @("-m", "pip", "install", "-U", "yt-dlp[default,curl-cffi]"))

Write-Step "Dependency check"
Invoke-Logged $pythonExe ($pythonArgs + @((Join-Path $PSScriptRoot "series_trailer_downloader.py"), "--check-deps"))

Write-Host ""
Write-Host "Done. You can start the GUI with:" -ForegroundColor Green
Write-Host "  python `"$PSScriptRoot\series_trailer_downloader.py`""
